...
import { mockPatients, mockKpis, mockTrends, mockAdherence } from '../utils/mock'
...
export const usePatients = (params = {}) => {
  return useQuery({
    ...
    queryFn: () => {
      ...
      return apiClient.get(`${API_ENDPOINTS.THERAPIST_PATIENTS}?${queryString}`)
    },
    staleTime: 5 * 60 * 1000,
  })
}
...
export const usePatientMetrics = (id, params = {}) => {
  return useQuery({
    ...
    queryFn: () => {
      const queryString = new URLSearchParams(params).toString()
      return apiClient.get(`${API_ENDPOINTS.PATIENT_DAILY_METRICS(id)}?${queryString}`)
    },
    enabled: !!id,
  })
}
...
export const useOverviewAdherence = (params = {}) => {
  return useQuery({
    ...
    queryFn: () => {
      ...
      const queryString = new URLSearchParams(params).toString()
      return apiClient.get(`${API_ENDPOINTS.OVERVIEW_ADHERENCE}?${queryString}`)
    },
  })
}
    ...
    queryFn: async () => {
      if (!FLAGS.PATIENT_KPIS_READY || ENABLE_MOCK) {
        ...
        const adherence7d = last7Days.length > 0
          ? last7Days.filter(d => d.medication_taken).length / last7Days.length
          : 0
...
export const useCreateDailyMetric = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, { patientId }) => {
      queryClient.invalidateQueries({ queryKey: ['patient-metrics', patientId] })
      queryClient.invalidateQueries({ queryKey: ['patient-kpis', patientId] })
    },
  })
}
...
export const useUpdateDailyMetric = () => {
  ...
  return useMutation({
    mutationFn: ({ patientId, metricId, data }) => 
      apiClient.put(`${API_ENDPOINTS.PATIENT_DAILY_METRICS(patientId)}/${metricId}`, data),
    onSuccess: (_, { patientId }) => {
      queryClient.invalidateQueries({ queryKey: ['patient-metrics', patientId] })
      queryClient.invalidateQueries({ queryKey: ['patient-kpis', patientId] })
    },
